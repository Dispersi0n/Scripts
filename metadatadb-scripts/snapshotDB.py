...
def addRoll(season,site,roll):
    return addRoll(season,site,roll,None,None)

def addRoll(season,site,roll,start,stop):
    ...
    if result == None:
        if start == None:
            ...
            command = ("INSERT INTO Rolls (Season,Site,RollNumber,StartDate,StopDate) " +
                       "VALUES (" + str(season) + "," + str(siteID) + "," +
                       str(roll) + ",\"" + str(start) + "\",\"" + str(stop) + "\")")
        execute(command)
        command = ("SELECT idRoll FROM Rolls WHERE Season=" + str(season) +
                   " AND Site=" + str(siteID) + " AND RollNumber=" + str(roll))
        execute(command)
...
def addManyUnlinkedZooniverseClassifications(afile):
    "Add a file's worth of classificaitons to the ZooniverseClassifications table"

    command = ("LOAD DATA LOCAL INFILE \"" + afile + "\" INTO TABLE "+
...
def addConsensusClassifications(afile):
...
def addCaptures(afile):
    "Add a file's worth of captures to the CaptureEvents table"

    command = ("LOAD DATA LOCAL INFILE \"" + afile + "\" INTO TABLE "+
               "CaptureEvents FIELDS TERMINATED BY ',' "+
               "ENCLOSED BY '' IGNORE 1 LINES " +
               "(Roll,CaptureEventNum,CaptureTimestamp,Invalid,ZooniverseStatus)")
...
def addImages(afile):
    "Add a file's worth of images to the Images table"

    command = ("LOAD DATA LOCAL INFILE \"" + afile + "\" INTO TABLE "+
               "Images FIELDS TERMINATED BY ',' "+
               "ENCLOSED BY '' IGNORE 1 LINES " +
               "(CaptureEvent,SequenceNum,PathFilename," +
               "TimestampAccepted,TimestampJPG) " +
               "SET TimestampAcceptedNote=\"Vetted by timestamp cleaning scripts\"")
...
def seasonExists(seasonNum):
    "Return true if the season is in the database, false if not"

    command = ("SELECT idSeason FROM Seasons WHERE idSeason=" + str(seasonNum))
    execute(command)
...
def siteExists(gridcell):
    "Return true if there is a current site for the given GridCell, false if not"

    command = ("SELECT idGridCell FROM GridCells JOIN Sites " +
               "ON GridCells.idGridCell=Sites.GridCell " +
               "WHERE GridCells.idGridCell=\"" + gridcell + "\" " +
               "AND Current=TRUE")
    execute(command)
...
def exportSeasonCaptureEvents(season):
    ...
    command = ("SELECT idCaptureEvent,Season,GridCell,RollNumber," +
               ...
               "JOIN Images ON Images.CaptureEvent=CaptureEvents.idCaptureEvent " +
               "WHERE Season="+season+" " +
               "AND ZooniverseStatus=1 " +
               "ORDER BY GridCell,RollNumber,CaptureEventNum,SequenceNum")
    execute(command)

    # big table! But shouldn't be a problem at MSI
    result = cur.fetchall()
